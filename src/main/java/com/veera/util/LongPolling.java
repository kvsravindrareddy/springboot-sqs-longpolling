...
//	private AmazonSQS amazonSQS;
	...
	public void longPollQueueMessages() throws ClassNotFoundException, IOException {
		...
		CreateQueueRequest create_request = new CreateQueueRequest().withQueueName("empobject")
		...
		SetQueueAttributesRequest set_attrs_request = new SetQueueAttributesRequest().withQueueUrl(empQueueUrl)
				.addAttributesEntry("ReceiveMessageWaitTimeSeconds", "20");
		amazonSQS.setQueueAttributes(set_attrs_request);

		// Enable long polling on a message receipt
		ReceiveMessageRequest receive_request = new ReceiveMessageRequest().withQueueUrl(empQueueUrl)
		...
		System.out.println("....standard polling messages size...."+receiveMsgs.size());
		for (Message resMsg : receiveMsgs) {
			...
			System.out.println("standard Response Body : " + emp.toString());
			deleteMsgs(amazonSQS, empQueueUrl, resMsg.getReceiptHandle());
		}
	}
	...
	public static void main(String[] args) {
		LongPolling obj = new LongPolling();
		try {
			obj.longPollQueueMessages();
